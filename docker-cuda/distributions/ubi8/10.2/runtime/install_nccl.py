...
def get_nccl_archive_file_name(version):
    return _nccl_version_archive_map[_nccl_sem_ver_version_map[version]]
...
def download_nccl(build_dir, download_url):
    os.chdir(build_dir)
    download_cmd = "wget -q --progress=bar:force {}".format(download_url)
...
def download_and_install_nccl(version, build_dir, install_dir):
    logging.info("Installing nccl version {} in directory {}".format(_nccl_sem_ver_version_map[version], install_dir))

    (installer, download_url) = get_nccl_download_location(version)
